  ...
  def onSuccess(evt: log.Success): F[Unit] = {
    ...
    scribe.debug(
      s"""Successful Statement Execution:
          ...
          |  ${renderSql(sql)}
          |  label = $label
          |  elapsed = ${executionTime.pretty} exec + ${processingTime.pretty} processing (${(executionTime + processingTime).pretty} total)
          |
          |${renderArgs(args)}
          |""".stripMargin
    )
  ...
  def onProcessingFailure(evt: log.ProcessingFailure): F[Unit] = {
    ...
    scribe.error(
      s"""Failed ResultSet Processing:
         ...
         |  ${renderSql(sql)}
         |
         |   elapsed = ${executionTime.pretty} exec + ${processingTime.pretty} processing (failed) (${(executionTime + processingTime).pretty} total)
         |   label = $label
  ...
  def onExecFailure(evt: log.ExecFailure): F[Unit] = {
    ...
    scribe.error(
      s"""Failed Statement Execution:
         |
         |  ${renderSql(sql)}
         |  elapsed = ${elapsed.pretty} exec (failed)
         |  label = $label
         |  failure = ${throwable.getMessage}
         |
         |${renderArgs(args)}
