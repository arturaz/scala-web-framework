...
import framework.exts.prettyFractional

/** Logs every executed Redis command at the given level. */
def LoggingCommandWrapper[F[_]: MonadCancelThrow: Clock](log: Scribe[F], level: Level): CommandWrapper[F] = new {
  override def wrap[A](name: String, attributes: collection.immutable.Iterable[Attribute[?]])(fa: F[A]): F[A] = {
    inline def commandStr = {
      val attrs = attributes.iterator.map(a => show"${a.key.name}=${a.value.toString()}").mkString("[\n", ",\n", "]\n")
    ...
    for {
      ...
      a <- fa.guaranteeCase { outcome =>
        for {
          ...
          durationStr = duration.prettyFractional
          _ <- outcome match {
