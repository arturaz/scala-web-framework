...
/** WebPush subscription as obtained from the client via
  ...
  */
case class WebPushSubscription(endpoint: Uri, keys: WebPushSubscription.Keys) derives CanEqual, Schema, CirceCodec {
  override def toString: String = s"WebPushSubscription(endpoint=$endpoint, $keys)"
}
object WebPushSubscription {
  case class Keys(auth: String, p256dh: String) derives CanEqual, Schema, CirceCodec {
