...
trait NewtypeUUID extends Newtype.WithoutValidationOf[UUID] {
  ...
  def fromBytes(bytes: IArray[Byte]): Either[String, Type] = {
    if (bytes.length != 16) Left(show"Failed to create UUID from bytes: expected 16 bytes, got ${bytes.length}")
    else
      Try {
        val bb = ByteBuffer.wrap(bytes.unsafeArray)
        apply(new UUID(bb.getLong(), bb.getLong()))
      }.toEither.left
        .map(err => show"Failed to create UUID from bytes: ${err.getMessage}")
  }

  def fromBase64(base64: Base64Id): Either[String, Type] =
    fromBytes(base64.bytes)

  /** Parses a string generated by [[Show]]. */
  def fromString(str: String): Either[String, Type] =
    Base64Id.fromBase64(str).toRight(show"Failed to decode base64: \"$str\"").flatMap(fromBase64)
