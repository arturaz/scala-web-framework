...
extension (obj: CirceCodec.type) {
  ...
  /** Provide an instance of [[CirceCodec]] for [[A]] if they implement [[CirceKeyEncoder]] and [[CirceKeyDecoder]]. */
  ...
  /** Provide an instance of [[CirceCodec]] for [[A]] if they implement [[TapirCodec]]. */
  def fromTapirCodec[LowLevel, HighLevel](using
    codec: TapirCodec[LowLevel, HighLevel, TapirCodecFormat.TextPlain],
    circeDecoder: CirceDecoder[LowLevel],
    circeEncoder: CirceEncoder[LowLevel],
  ): CirceCodec[HighLevel] =
    CirceCodec.from(
      circeDecoder.emap(codec.decode(_).toEither.left.map(err => err.toString())),
      circeEncoder.contramap(codec.encode),
    )
}
